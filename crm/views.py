...
def summary(request, pk):
    ...
    # total__amt = sum_service_charge.get("service_charge__sum") + sum_product_charge.get("charge__sum")
    # sum_total = {'total__key' : total__amt }
...
def export_csv(request, pk):
    ...
    response['Content-Disposition'] = 'attachment; filename="ServiceSummary.csv"'
    customer = get_object_or_404(Customer, pk=pk)
    services = Service.objects.filter(cust_name=pk)
    products = Product.objects.filter(cust_name=pk)

    writer = csv.writer(response)
    writer.writerow(['Customer', 'Category', 'Description', 'Set Up', 'Clean Up', 'Service Charge'])

    # customers = Customer.objects.filter(created_date__lte=timezone.now())
    for service in services:
        writer.writerow([service.cust_name, service.service_category, service.description,
                        service.setup_time, service.cleanup_time, service.service_charge])

    return response

@login_required
def export_prod_csv(request, pk):
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="ProductSummary.csv"'
    # customer = get_object_or_404(Customer, pk=pk)
    # services = Service.objects.filter(cust_name=pk)
    products = Product.objects.filter(cust_name=pk)

    writer = csv.writer(response)
    writer.writerow(['Customer', 'Product', 'Description', 'Quantity', 'Pick Up', 'Product Charge'])

    # customers = Customer.objects.filter(created_date__lte=timezone.now())
    for product in products:
        writer.writerow([product.cust_name, product.product, product.p_description,
                        product.quantity, product.pickup_time, product.charge])
